...
class Driver(driver_grpc.DriverServicer):
    def __init__(self):
        super().__init__()
        self.workers = {}
        self.iterations = 5  # Number of iterations
    ...
    def launchDriver(self, request, context):
        def initialize_centroids(num_clusters, dimension):
            ...
            self.centroids = np.random.rand(num_clusters, dimension) * 100  # Random initialization
            print("Initial Centroids:")
            print(self.centroids)

        def map_kmeans(key, file, mid, num_clusters):
            print(f"[*] [DRIVER] [MAP] Map operation '{mid}' on the file '{file}' is sent to worker '{key}'...")
            self.workers[key][0] = 1
            r = worker.kmeansInput(path=file, mapID=mid, numClusters=num_clusters, centroids=self.centroids)
        ...
        print("[!] [DRIVER] [CONFIG]  Registered: %i map operations and %i reduce operations" % (len(files), num_clusters))

        print("[!] [DRIVER] [KMEANS] Initializing centroids...")
        initialize_centroids(num_clusters, dimension)

        for _ in range(self.iterations):
            print(f"[!] [DRIVER] [KMEANS] Starting iteration {_+1}/{self.iterations}")
            ...
            with futures.ThreadPoolExecutor() as executor:
                for idx, file in enumerate(files):
                    ...
                    executor.submit(map_kmeans, key=tmp_worker, file=file, mid=idx, num_clusters=num_clusters)
            ...
            with futures.ThreadPoolExecutor() as executor:
                for idx in range(num_clusters):
                    ...
                    while tmp_worker is False:
                        ...
                        time.sleep(5)
                        tmp_worker = get_worker()
                    executor.submit(reduce_kmeans, key=tmp_worker, rid=idx)
